...
class AutoEncoder(object):
  ...
  def derivadaAtivacaoSigmoidall(self, valor):
  ...
  def loss(self, entrada, saida):
      # Calcula o erro quadrático médio (MSE)
      mse = tf.reduce_mean(tf.square(entrada - saida))
  ...
  def backPropagation(self, dados, esperado, saidas_oculta, saidas_final):
    ...
    grad_saida = np.array([])
    for i in range(self.saida):
        grad_saida = np.append(grad_saida, (valores_esperados[i] - saidas_final[i]) * self.derivadaAtivacaoSigmoidall(saidas_final[i]))

    # grad_oculta = []
    # for j in range(self.oculta):
    #     grad_oculta.append(self.derivadaAtivacaoSigmoidall(saidas_oculta[j]) * np.sum(grad_saida[:, np.newaxis] * self.Wo[:,j]))
    grad_oculta = []
    for j in range(self.oculta):
        soma_ponderada = np.sum(grad_saida * self.Wo[:,j])
        grad_oculta.append(self.derivadaAtivacaoSigmoidall(saidas_oculta[j]) * soma_ponderada)
  ...
  def treinamento(self, dados, esperado, epocas):
    ...
    for epoca in range(epocas):
      ...
      for i, exemplo in enumerate(dados):
        ...
        erro.append(self.loss(exemplo,saidas_final))
...
if __name__ == '__main__':  
  ...
  m = AutoEncoder(4, 2, 4, taxaDeAprendizado=lr)
  ...
  # m = AutoEncoder(4, 2, 4, taxaDeAprendizado=0.1)
